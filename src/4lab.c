...
int request_cs(const void *self) {
    ...
    if (queue == NULL) queue_init(&queue);

    queue_push(queue, info->localID, get_lamport_time());
    int res = send_multicast(info, &msg);
    if (res != EXIT_SUCCESS) return res;

    int repliesCount = info->nChild - 1;

    while (repliesCount > 0 || queue->front->pid != info->localID) {
        ...
        switch (msg.s_header.s_type) {
            case CS_REQUEST:
                queue_push(queue, info->lastMsgPid, msg.s_header.s_local_time);
                increment_lamport_time();
